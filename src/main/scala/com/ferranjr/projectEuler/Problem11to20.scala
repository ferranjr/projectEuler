...
object Problem11 {
  ...
  val horizontal: Seq[(Int, Int)] = Seq((0,0),(1,0),(2,0),(3,0))
  val vertical:   Seq[(Int, Int)] = Seq((0,0),(0,1),(0,2),(0,3))
  val diagonal1:  Seq[(Int, Int)] = Seq((0,0),(1,1),(2,2),(3,3))
  val diagonal2:  Seq[(Int, Int)] = Seq((0,3),(1,2),(2,1),(3,0))

  val increments: Seq[Seq[(Int, Int)]] = Seq(horizontal, vertical, diagonal1, diagonal2)

  def getMaxProduct(gr: Vector[Vector[Int]]):Long = {
    (
      for{
        xPos  <- 0 until width  - 3
        yPos  <- 0 until height - 3
        inc   <- increments
      }
      yield
        gr(yPos + inc.head._2)(xPos + inc.head._1 ) *
        gr(yPos + inc(1)._2  )(xPos + inc(1)._1   ) *
        gr(yPos + inc(2)._2  )(xPos + inc(2)._1   ) *
        gr(yPos + inc(3)._2  )(xPos + inc(3)._1   )
    )
    .max
  }
